...
function createtime2() {
  ...
  setTimeout(
    console.warn.bind(
      console,
      "%c ⚡ Powered by XUZI'BLOG %c 你正在访问A-XUZIの小家",
...
function share_() {
  ...
  try {
    ...
    var subTitle = title.endsWith("| XUZI'BLOG") ? title.substring(0, title.length - 14) : title;
    navigator.clipboard.writeText('A-XUZI的站内分享\n标题：' + subTitle + '\n链接：' + url + '\n欢迎来访！🍭🍭🍭');
    new Vue({
      data: function () {
        this.$notify({
          title: "成功复制分享信息🎉",
